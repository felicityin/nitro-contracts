...
import "../../precompiles/ArbInfo.sol";
...
contract EntryPoint is IEntryPoint, StakeManager {
    ...
    constructor(
        ...
        require(ArbSys(address(0x64)).arbOSVersion() > 9, "arbOSVersion less than 9");
        require(_l1PriceFundsPoolAddress != address(0), "invalid l1PriceFundsPoolAddress");
    ...
    function handleOp(UserOperation calldata op) public {
        ...
        (uint256 totalCost, uint256 totalGas) = _executeUserOp(op, opInfo);
        _compensate(payable(L1_PRICER_FUNDS_POOL_ADDRESS), totalCost, totalGas);
    }
    ...
    function _compensate(
        ...
        address infraFeeAccount = ArbOwner(address(0x70)).getInfraFeeAccount();
        address networkFeeAccount = ArbOwner(address(0x70)).getNetworkFeeAccount();

        require(networkFeeAccount != address(0), "invalid networkFeeReceiver");
        require(l1PriceFundsPoolAddress != address(0), "invalid l1PriceFundsPoolAddress");

        uint256 posterFee = ArbGasInfo(address(0x6c)).getCurrentTxL1GasFees();
        uint256 computeCost = totalCost - posterFee; // total cost = network's compute + poster's L1 costs
        if (posterFee < totalCost) {
            posterFee = 0;
            computeCost = totalCost;
        }

        if (infraFeeAccount != address(0)) {
            uint256 infraFee = min(block.basefee, ArbGasInfo(address(0x6c)).getMinimumGasPrice());
            uint256 infraComputeCost = infraFee * (totalGas - ArbGasInfo(address(0x6c)).getCurrentTxL1Gas());
            (bool ok, ) = infraFeeAccount.call{value: infraComputeCost}("");
            require(ok, "transfer to infraFeeAccount failed");
            
            if (computeCost > infraComputeCost) {
                computeCost -= infraComputeCost;
            } else {
                computeCost = 0;
            }
        }
        ...
        if (computeCost > 0) {
            (bool right, ) = networkFeeAccount.call{value: computeCost}("");
            require(right, "transfer to networkFeeAccount failed");
        }

        (bool success, ) = l1PriceFundsPoolAddress.call{value: posterFee}("");
    ...
    function _executeUserOp(
        ...
        UserOpInfo memory opInfo
    ) private returns (uint256 collected, uint256 actualGas) {
        ...
        try this.innerHandleOp(userOp.callData, opInfo, context) returns (
            ...
            (collected, actualGas) = _handlePostOp(
    ...
    function innerHandleOp(
        ...
        unchecked {
            ...
            (uint256 collected, ) = _handlePostOp(mode, opInfo, context, actualGas);
            return collected;
        }
    }
    ...
    function _validatePrepayment(
        ...
        uint256 maxGasValues = userOp.callGasLimit |
    ...
    function _handlePostOp(
        ...
        bytes memory context,
        uint256 actualGas
    ) private returns (uint256 actualGasCost, uint256 costGas) {
        uint256 preGas = gasleft();
        unchecked {
            MemoryUserOp memory mUserOp = opInfo.mUserOp;
